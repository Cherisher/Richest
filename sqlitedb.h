...
public:
    ...
    SQLiteDB(const char *path, int flags = SQLITE_OPEN_READONLY, const char *zVfs = NULL) : conn(NULL)
    {
    ...
    long long execInt(const char *sql)
    {
        ...
        if (!c.read() || c.getNumCols() < 1 || !c[0])
            return -1;

        return strtoll(c[0], NULL, 10);
    }

    double execDouble(const char *sql)
    {
        Cursor c(*this, sql);
    ...
    {
    public:
        Cursor(SQLiteDB &_db, const char *sql) : db(_db), stmt(NULL), pos(0)
        {
        ...
        SQLiteDB &getDB() const { return db; }
        ...
        const char *getVal(const char *col) const { return getVal(getColIdx(col)); }

        const char *operator[](int idx) const { return getVal(idx); }

        const char *operator[](const char *col) const { return getVal(col); }
    ...
    std::unique_ptr<Cursor> execCursor(const char *sql) { return std::unique_ptr<Cursor>(new Cursor(*this, sql)); }
