...
void init_db()
{
	try {
		...
		db.exec("create table if not exists wallet_info(id INTEGER PRIMARY KEY AUTOINCREMENT, address TEXT NOT NULL, amount REAL DEFAULT 0.0)");
...
double getBalance(wfcClient &client, const std::string &address)
{
	...
	try {
		...
		cout << "getaddressbalance: " << address << " failed: " << e.GetMessage() << endl;
	}
	return balance;
}
		...
		try {
			...
			for(const auto &wallet : wallets){
				...
				if(sqlite3_step(stmt) != SQLITE_DONE) {
					cerr << "Error insert " << ": " << wallet.address << "\t" <<  wallet.balance << endl;
				}
			}
			db.exec("COMMIT;");
		}catch(...){
